...
const INITIAL_VISIBLE_COLUMNS = ["firstName","date","time", "actions"];
...
export default function TableComponent({callToAction}:{callToAction:string}) {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((user) =>
        user.employeeId.firstName.toLowerCase().includes(filterValue.toLowerCase()),
      );
    }
  ...
  const renderCell = React.useCallback((user: User, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      case "firstName":
        return (
          <User
            avatarProps={{radius: "full", size: "sm", src: user.employeeId.imageUrl}}
            classNames={{
              description: "text-default-500",
            }}
            description={user.employeeId.email}
      ...
      case "tagNumber":
        return (
          <p>{user.employeeId.tagNumber}</p>
        );
      case "date":
        const processedTime = convertTimestamp(user.createdAt)
        return (
          <p>{processedTime.dateOnly}</p>
        );
      case "time":
        const processedTime2 = convertTimestamp(user.createdAt)
        return (
          <p>{processedTime2.timeOnly}</p>
        );
      case "actions":
        return (
 ...
 async function deleteCheckedinEmployee(employeeId:any){
  console.log("pressed")
  const id = {
                  ...
                  <ModalBody>
                    <div className="space-y-2">
                      <div className="flex flex-row items-center justify-center space-x-2">
                        <Image src={employeeDetails.employeeId.imageUrl} alt="profile picture" width={100} height={100} className="rounded-full"></Image>
                        <div className="">
                          <p className="headingsix font-medium text-gray-800">{`${employeeDetails.employeeId.firstName} ${employeeDetails.employeeId.lastName}`}</p>
                          <p className="text-gray-600">{`${employeeDetails.employeeId.email}`}</p>
                        </div>
                      </div>
                      <div className="pl-8">
                        <p className="text-gray-600 font-medium">Tag Number:<span className="ml-2 text-gray-800">{employeeDetails.employeeId.tagNumber}</span></p>
                        <p className="text-gray-600 font-medium">Id Number:<span className="ml-2 text-gray-800">{employeeDetails.employeeId.idNumber}</span></p>
                        <p className="text-gray-600 font-medium">status:<span className="ml-2 text-gray-800">{employeeDetails.employeeId.status}</span></p>
                      </div>
            ...
            <ModalContent>
              {(onClose) => (
                <>
                  ...
                  <ModalBody>
                    <p>Are you sure you would like to delete the employee <span className="headingsix font-medium text-gray-600">{`${employeeDetails.employeeId.firstName} ${employeeDetails.employeeId.lastName}`}</span></p>
                  </ModalBody>
                  <ModalFooter>
                    <Button color="danger" variant="light" onClick={()=>{
                      deleteCheckedinEmployee(employeeDetails._id)
                      onClose()
                    }}>
                      Delete
                    </Button>
